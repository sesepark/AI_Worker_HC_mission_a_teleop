...
> - ❌ `perception_2d_to_pcd_wrist/.../wrist_task_grasp_planner_node.py` + `wrist_task_grasp_planner.launch.py` (cherry-pick된 최신 planner)
> - ❌ `perception_part_detector/weights/best.pt`, `monitor_ocr/best.pt` (모델 가중치)
...
ls ~/robotis_ros2_ws/src/perception_2d_to_pcd_wrist/perception_2d_to_pcd_wrist/wrist_task_grasp_planner_node.py
ls ~/robotis_ros2_ws/src/perception_part_detector/weights/best.pt
ls ~/robotis_ros2_ws/src/monitor_ocr/best.pt
```
...
# yolo_venv  (perception_part_detector: ultralytics→torch 자동, cv2, numpy)
...
ls /ws/install/perception_part_detector/share/perception_part_detector/weights/best.pt
ls /ws/install/monitor_ocr/share/monitor_ocr/best.pt
```

빌드 실패 시 자주 보는 에러:
- `perception_part_detector messages not built` → `perception_part_detector` 가 다른 노드보다 먼저 빌드되어야 함. `--packages-select` 순서대로 강제: `colcon build --packages-up-to perception_2d_to_pcd_wrist`
...
| 터미널 | 추가 venv | 실행 |
|--------|----------|------|
| T1 (OCR) | `source /ws/ocr_venv/bin/activate` | `ros2 run monitor_ocr monitor_ocr_node --ros-args -p parts_mode:=true` |
| T2 (Detector) | `source /ws/yolo_venv/bin/activate` | `ros2 launch perception_part_detector detector.launch.py camera_name:=wrist_right` |
| T3 (Projection) | (system) | `ros2 launch perception_2d_to_pcd_wrist wrist_projection.launch.py` |
| T4 (Pointcloud) | (system) | `ros2 launch perception_2d_to_pcd_wrist wrist_pointcloud.launch.py` |
| T5 (Grasp PCD) | (system) | `ros2 launch perception_2d_to_pcd_wrist wrist_grasp_pcd.launch.py` |
| T6 (Planner) ⭐ | (system) | `ros2 run perception_2d_to_pcd_wrist wrist_task_grasp_planner_node --ros-args --params-file /ws/src/perception_2d_to_pcd_wrist/config/params.yaml` |
...
| `cannot import name 'PartDetectionArray'` | 메시지 패키지 빌드 순서 | `colcon build --packages-up-to perception_2d_to_pcd_wrist` |
...
| best.pt 못 찾음 | colcon install share 위치 차이 | `--ros-args -p model_path:=/ws/src/perception_part_detector/weights/best.pt` 로 명시 |
...
/ws/ocr_venv/bin/python /ws/install/monitor_ocr/lib/monitor_ocr/monitor_ocr_node \
  --ros-args -p parts_mode:=true
# launch 도 제공됨: ros2 launch monitor_ocr monitor_ocr.launch.py
...
ros2 launch perception_part_detector detector.launch.py camera_name:=head        # head
ros2 launch perception_part_detector detector.launch.py camera_name:=wrist_left  # wrist L
ros2 launch perception_part_detector detector.launch.py camera_name:=wrist_right # wrist R
...
# (옵션) ros2 run perception_2d_to_pcd pointcloud_node / grasp_pcd_node
# 확인:
...
ros2 launch perception_2d_to_pcd_wrist wrist_projection.launch.py    # wrist_projection_node
ros2 launch perception_2d_to_pcd_wrist wrist_pointcloud.launch.py    # wrist_pointcloud_node
# 최종 target 1개 (planner) — launch + params_file
ros2 launch perception_2d_to_pcd_wrist wrist_task_grasp_planner.launch.py \
  params_file:=/ws/src/perception_2d_to_pcd_wrist/config/params.yaml
```
...
### A-6. task_management — 트레이 검출 + 잔여 task list (신규 `demo/senario_A`)
```bash
...
> `task_management.launch.py`)은 `prefix="/ws/ocr_venv/bin/python3"` 처럼 venv python을 직접 지정해
> 노드를 띄운다. 그래서 **launch로 실행하면 `ros2 run` shebang 문제가 안 생긴다.**
> ```bash
> ros2 launch monitor_ocr monitor_ocr.launch.py   # ocr_venv prefix 내장 — 함정 ① 우회
> ```
