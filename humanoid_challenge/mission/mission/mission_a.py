...
검증된 토픽 파이프라인: tray_manage_node → /perception/task_list (mission_interfaces GetTaskList.Response).
"""
...
class MissionA(Node):
    def __init__(self) -> None:
        ...
        self.sub_attached_object = self.create_subscription(
            ...
            callback_group=self._cbg)
        # task list 입력 = tray_manage_node /perception/task_list (GetTaskList.Response).
        #   (perception-new 타입드 토픽 계약 — 통합 시 String-JSON 에서 전환)
    ...
    def _on_task_list(self, msg: GetTaskList.Response) -> None:
        ...
        - INIT/A1_MONITOR 단계에서만 목표 task_list 를 (재)빌드(이후 자체 차감 보호).
