...
def generate_launch_description() -> LaunchDescription:
    ...
    args = [
        ...
        # base_seq: 카메라 픽(use_camera) + 너트 슬롯→픽 / 파이프 A·B→place 로 베이스 측방+전진 이동.
        #   검증용: insert_dry_run:=true 면 place 는 dry(베이스 A/B 도달까지만 확인).
        #   nav_mode:=service + 로봇 PC move_base_lateral 서버 필수(베이스 실제 이동).
        DeclareLaunchArgument('base_seq_enable', default_value='false',
                              description='너트별 측방정렬 픽 + A/B(측방+전진) place 시퀀스. '
                                          'true 면 nav_mode:=service 필수'),
        DeclareLaunchArgument('use_camera', default_value='true',
                              description='true=perception 픽(pick_class_order). false=하드코딩 nocam 픽'),
        DeclareLaunchArgument('nut_pitch_mm', default_value='150.0',
                              description='base_seq 너트 슬롯 간 측방 간격(mm)'),
        DeclareLaunchArgument('place_forward_mm', default_value='100.0',
                              description='base_seq place 전진량(mm)'),
