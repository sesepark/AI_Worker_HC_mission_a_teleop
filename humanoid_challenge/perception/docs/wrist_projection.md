...
패키지입니다. head 패키지(`perception`)의 wrist 버전입니다.
...
| `/detections` | `perception/PartDetectionArray` |
| `/tf`, `/tf_static` | TF |
...
# 또는 따로따로
ros2 launch perception wrist_projection.launch.py
ros2 launch perception wrist_pointcloud.launch.py
ros2 launch perception wrist_grasp_pcd.launch.py   # 핵심
```
...
4. **빌드 순서** — `perception`가 먼저 빌드되어 있어야 합니다.
