...
#
#  사용법:
#    bash ~/ai_worker/perception/tools/run_monitor_ocr.sh          # 로컬 실행
#    bash ~/ai_worker/perception/tools/run_monitor_ocr.sh docker   # docker 컨테이너 실행
#
#  옵션 (환경변수로 재정의 가능):
...
echo "[*] OCR 노드 시작..."
echo "    결과 확인: ros2 service call /mission_a/task_list mission_interfaces/srv/GetTaskList \"{timeout_sec: 20.0, frame_count: 3}\""
echo "    종료    : Ctrl+C"
echo ""
...
if [ "$MODE" = "docker" ]; then
    ...
    docker exec -it ai_worker bash -c "
        ...
        ros2 run perception monitor_ocr_node --ros-args \
    ...
    if [ -n "$WS" ]; then
        ...
        echo "    cd ~/ros2_ws && colcon build --packages-up-to perception"
        exit 1
    fi
